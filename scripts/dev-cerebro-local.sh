...
echo ""
echo "  Logs API:       tail -f $LOG"
echo "  Logs Vite:      tail -f $VITE_LOG"
echo "  Logs ambos:     tail -f $LOG $VITE_LOG"
echo ""
echo "  Login: botón «Entrar (dev local)»"
echo "  Consola browser: filtrar [cerebro-dev] para tráfico cliente"
...
cd "$APP"
: >"$VITE_LOG"
npm run dev --prefix src 2>&1 | tee -a "$VITE_LOG"
