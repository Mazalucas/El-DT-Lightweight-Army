...
2. **Confirm the target slugs cleanly**:
   ```bash
   tools/atelier/generated/scripts/impeccable critique-storage slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
tools/atelier/generated/scripts/impeccable detect --json [target]
```
...
4. If mutation is available, start `tools/atelier/generated/scripts/impeccable live-server --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
- **Suggested command**: Which command could address this (from: /atelier adapt, /atelier animate, /atelier audit, /atelier bolder, /atelier clarify, /atelier colorize, /atelier critique, /atelier delight, /atelier distill, /atelier document, /atelier harden, /atelier layout, /atelier onboard, /atelier optimize, /atelier overdrive, /atelier polish, /atelier quieter, /atelier shape, /atelier typeset)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `.agents/design-context.md` contains a `## Design Context` section from `impeccable init`, also generate 1-2 project-specific personas from the audience/brand info.
...
Once the report above is finalized, write it to `.impeccable/critique/` so the user can refer back, and so `/atelier polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     tools/atelier/generated/scripts/impeccable critique-storage write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   tools/atelier/generated/scripts/impeccable critique-storage trend "<resolved target>" 5
   ```
...
1. **`/atelier command-name`**: Brief description of what to fix (specific context from critique findings)
2. **`/atelier command-name`**: Brief description (specific context)
...

**Rules for recommendations**:
- Only recommend commands from: /atelier adapt, /atelier animate, /atelier audit, /atelier bolder, /atelier clarify, /atelier colorize, /atelier critique, /atelier delight, /atelier distill, /atelier document, /atelier harden, /atelier layout, /atelier onboard, /atelier optimize, /atelier overdrive, /atelier polish, /atelier quieter, /atelier shape, /atelier typeset
...
- End with `/atelier polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `/atelier critique` after fixes to see your score improve.
...
If `.agents/design-context.md` contains a `## Design Context` section (generated by `impeccable init`), derive 1–2 additional personas from the audience and brand information:
