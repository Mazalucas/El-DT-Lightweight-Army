...
- **Tool version.** The installed skill is older than the published one. `context.adapter.mjs` reports that at boot as `UPDATE_AVAILABLE` and `/atelier actualizar` fixes it. Not this command's job.
...
```
tools/atelier/generated/scripts/impeccable doctor --json
```
...
- **`auto`** carries no decision. Run `tools/atelier/generated/scripts/impeccable doctor --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
