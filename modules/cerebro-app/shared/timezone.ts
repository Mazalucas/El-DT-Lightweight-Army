...
function fallbackTimezone(settings: UserAppSettings): string {
  return pickValidTimezone(
    settings.locale?.timezone,
    settings.syncSchedule?.timezone,
...
/** TZ efectiva en cliente (combina settings + navegador). */
export function resolveClientTimezone(settings: UserAppSettings | undefined): string {
  if (!settings) return browserTimezone();
  const source = settings.locale?.timezoneSource ?? 'device';
  if (source === 'device') {
    return browserTimezone();
  }
  return resolveUserTimezone(settings, browserTimezone());
}
