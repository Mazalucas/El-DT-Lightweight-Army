...
export interface Team {
  ...
  color: string;
  tags?: string[];
  /** Emails de equipo / Google Group — no crean Persona al reindexar. */
  emails?: string[];
}
...
export interface CerebroSettingsPrefs {
  ...
  /** UI viva: optimistic elements, dnd-kit, entity effects from Cerebro. */
...
/** Metadatos opcionales al confirmar un prospect (alias, equipos, proyectos). */
export interface ProspectResolveEnrichment {
  aliases?: string[];
  teamIds?: string[];
  projectIds?: string[];
}
...
/** Respuesta mínima de mutaciones de tareas (Camino A / Cerebro Elements). */
export interface TodoMutationResult {
  todo?: MeetingTodo;
  todos?: MeetingTodo[];
  meta?: { counts: BoardCounts };
  /** Compat rollout — omitido cuando el cliente no lo necesita. */
  store?: CerebroStore;
}
...
export interface StoreHealthMetrics {
  ...
  /** personIds/prospectIds en reuniones que no existen en catálogo */
  ...
  /** Proyectos con id UUID (legado IA) — requieren repair */
  uuidProjects?: number;
  contactsCount: number;
  /** true si conviene ejecutar repair (re-sync mirrors + import) */
  needsRepair?: boolean;
