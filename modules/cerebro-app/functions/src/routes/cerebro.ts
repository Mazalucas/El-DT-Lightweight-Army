...
import type { CerebroClientContextInput, CerebroSseEvent } from '../shared/cerebro-chat.js';
...
function writeSse(res: import('express').Response, event: CerebroSseEvent): void {
  res.write(`data: ${JSON.stringify(event)}\n\n`);
}

cerebroRouter.get('/status', async (req: AuthedRequest, res) => {
  try {
...
cerebroRouter.get('/context', async (req: AuthedRequest, res) => {
  try {
    const uid = getUid(req);
    const client = parseClientContext(undefined, req.query as Record<string, unknown>);
    const dismissed = parseDismissed(req.query as Record<string, unknown>);
...
cerebroRouter.post('/context', async (req: AuthedRequest, res) => {
  try {
    const uid = getUid(req);
    const body = (req.body ?? {}) as {
      clientContext?: CerebroClientContextInput;
      dismissedMomentKeys?: string[];
      userTyping?: boolean;
      toolRunning?: boolean;
      planPending?: boolean;
    };
...
cerebroRouter.post('/moments/dismiss', async (req: AuthedRequest, res) => {
  try {
    const uid = getUid(req);
    const { conversationId, momentKey } = (req.body ?? {}) as {
...
cerebroRouter.get('/conversations', async (req: AuthedRequest, res) => {
  try {
...
cerebroRouter.get('/conversations/:id', async (req: AuthedRequest, res) => {
  try {
...
cerebroRouter.delete('/conversations/:id', async (req: AuthedRequest, res) => {
  try {
...
cerebroRouter.post('/chat', async (req: AuthedRequest, res) => {
  try {
    const uid = getUid(req);
    const { message, conversationId, clientContext } = (req.body ?? {}) as {
      message?: string;
      conversationId?: string;
      clientContext?: CerebroClientContextInput;
    };
...
function parseDismissed(query: Record<string, unknown>): string[] | undefined {
...
function parseClientContext(
  ...
  const fromBody = (body as { clientContext?: CerebroClientContextInput } | undefined)?.clientContext;
  if (fromBody?.navigation) return fromBody;
