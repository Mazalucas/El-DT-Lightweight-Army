...
import type { ApiStatus, SyncProgress, SyncProgressResponse } from '../shared/types.js';

function normalizeSyncProgress(progress: SyncProgress | null): SyncProgress | null {
...
syncRouter.post('/scan', async (req: AuthedRequest, res) => {
  try {
...
syncRouter.post('/run', async (req: AuthedRequest, res) => {
  try {
    const uid = getUid(req);
...
syncRouter.get('/progress', async (req: AuthedRequest, res) => {
  try {
...
syncRouter.post('/pipeline', async (req: AuthedRequest, res) => {
  try {
    const uid = getUid(req);
    const { mode, progress } = await resolveSyncStartMode(uid);
    if (mode === 'join') {
    ...
    const { limit, skipAnalysis } = (req.body ?? {}) as { limit?: number; skipAnalysis?: boolean };
    const startedAt = await markSyncStarting(uid, {
      phase: 'pipeline',
      current: 0,
      total: 4,
...
meetingsRouter.get('/:id/content', async (req: AuthedRequest, res) => {
  try {
...
meetingsRouter.post('/import', async (req: AuthedRequest, res) => {
  try {
...
storeRouter.get('/', async (req: AuthedRequest, res) => {
  try {
...
storeRouter.put('/', async (req: AuthedRequest, res) => {
  try {
...
storeRouter.get('/summary', async (req: AuthedRequest, res) => {
  try {
    const uid = getUid(req);
...
storeRouter.get('/meetings', async (req: AuthedRequest, res) => {
  try {
