...
export async function listLlmProviders(uid: string): Promise<LlmProviderMeta[]> {
  const ids: LlmProviderId[] = ['google_gemini', 'openai'];
  const result: LlmProviderMeta[] = [];
  for (const providerId of ids) {
    ...
    const data = snap.data() as { keyHint: string; modelDefault?: string; enabled?: boolean; lastValidatedAt?: string; lastError?: string };
    result.push({
      providerId,
      label: defaults.label,
...
export async function setLlmProviderKey(uid: string, providerId: LlmProviderId, apiKey: string, modelDefault?: string): Promise<LlmProviderMeta> {
...
export async function deleteLlmProviderKey(uid: string, providerId: LlmProviderId): Promise<void> {
  await llmProviderRef(uid, providerId).delete();
}

async function getProviderKey(uid: string, providerId: LlmProviderId): Promise<{ key: string; model: string }> {
  const snap = await llmProviderRef(uid, providerId).get();
  if (!snap.exists) throw new Error(`No API key for ${providerId}`);
  const data = snap.data() as { encryptedKey: string; modelDefault: string; enabled?: boolean };
  if (data.enabled === false) throw new Error(`Provider ${providerId} disabled`);
  return { key: decrypt(data.encryptedKey), model: data.modelDefault };
}

export async function testLlmKey(providerId: LlmProviderId, apiKey: string, model?: string): Promise<void> {
  if (providerId === 'google_gemini') {
    ...
    if (!r.response.text()) throw new Error('Empty response from Gemini');
    return;
  }
  if (providerId === 'openai') {
    ...
    if (!r.choices[0]?.message?.content) throw new Error('Empty response from OpenAI');
    return;
  }
  throw new Error(`Unknown provider ${providerId}`);
}

export async function testStoredLlmKey(uid: string, providerId: LlmProviderId): Promise<void> {
...
async function callLlm(uid: string, prompt: string): Promise<string> {
  const { callUserLlmText } = await import('./llm-service.js');
  return callUserLlmText(uid, prompt, { temperature: 0.3 });
}

export async function analyzeMeeting(uid: string, meetingId: string): Promise<AnalysisPayload> {
  const md = await getMirrorContent(uid, meetingId);
  if (!md) throw new Error('Meeting mirror not found — sync first');
  const body = md.replace(/^---[\s\S]*?---\n/, '').trim();
  const prompt = `${ANALYSIS_PROMPT}\n\nmeetingId: ${meetingId}\n\nContenido:\n${body.slice(0, 120000)}`;
  const raw = await callLlm(uid, prompt);
  const json = JSON.parse(raw.replace(/^```json?\s*|\s*```$/g, '').trim()) as AnalysisPayload;
...
export async function applyAnalysisToStore(uid: string, analysis: AnalysisPayload): Promise<CerebroStore> {
...
export async function getJob(uid: string, jobId: string) {
  const snap = await jobsCol(uid).doc(jobId).get();
  return snap.exists ? snap.data() : null;
}

export async function loadStore(uid: string): Promise<CerebroStore> {
  return loadStoreFromRepository(uid);
}

export async function saveStore(uid: string, store: CerebroStore): Promise<void> {
