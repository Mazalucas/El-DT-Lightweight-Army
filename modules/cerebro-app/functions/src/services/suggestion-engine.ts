...
import {
  buildMeetingPrepFacts,
  buildTemplateMeetingPrepInsights,
} from '../domain/meeting-prep-insights.service.js';
import type { MeetingPrepFact, MeetingPrepInsight } from '../shared/types.js';
...
function recentMeetings(store: CerebroStore): Meeting[] {
  ...
  return sortMeetingsByRecency(
    store.meetings.filter((m) => m.startedAt && m.startedAt >= cutoff),
...
export async function generateDailyDigest(uid: string): Promise<DailyDigest | null> {
  ...
  const upcomingEvents =
    calendarToday?.hasCalendarAccess && calendarToday.events.length
      ? calendarToday.events.filter((ev) => ev.status !== 'past')
      : [];
  const prepFacts: MeetingPrepFact[] =
    calendarToday && upcomingEvents.length
  ...
  for (const t of daily.overdue.slice(0, 6)) lines.push(`- VENCIDA: "${t.text}" (${t.dueAt?.slice(0, 10)})`);
  lines.push(`Tareas para hoy: ${daily.today.length}`);
  for (const t of daily.today.slice(0, 6)) lines.push(`- HOY: "${t.text}"`);
  ...
  for (const m of meetings) {
    const openTodos = store.todos.filter(
      (t) => t.meetingId === m.id && (t.status === 'open' || t.status === 'suggested'),
    ).length;
    lines.push(
      `- ${m.title} (${m.startedAt?.slice(0, 10)})${openTodos ? ` · ${openTodos} tareas` : ''}${m.summary ? `: ${m.summary.slice(0, 160)}` : ''}`,
    );
  }
  lines.push('');
  ...
  const meetingPrepInsights: MeetingPrepInsight[] =
    prepFacts.length && upcomingEvents.length
  ...
  if (prepFacts.length) {
    console.info(
      `[intelligence] meeting prep insights uid=${uid} facts=${prepFacts.length} insights=${meetingPrepInsights.length}`,
    );
  }
  return digest;
}
