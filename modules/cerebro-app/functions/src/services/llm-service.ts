...
const PROVIDER_DEFAULTS: Record<LlmProviderId, { modelDefault: string }> = {
...
export interface LlmCallWithToolsResult {
  text?: string;
...
): Promise<{ providerId: LlmProviderId; key: string; model: string }> {
  const settings = await loadSettings(uid);
  const pid = providerId ?? (settings.ai.defaultProviderId as LlmProviderId);
  const snap = await llmProviderRef(uid, pid).get();
  if (!snap.exists) {
    throw new Error(
      `No hay API key configurada para ${pid}. Configurala en Ajustes → Proveedores IA.`,
    );
  }
  const data = snap.data() as { encryptedKey: string; modelDefault?: string; enabled?: boolean };
  if (data.enabled === false) throw new Error(`Proveedor ${pid} deshabilitado`);
  return {
    providerId: pid,
...
export async function userHasLlmKey(uid: string): Promise<boolean> {
  try {
...
export async function callUserLlmText(
  uid: string,
  prompt: string,
  opts?: { systemInstruction?: string; temperature?: number; maxTokens?: number },
): Promise<string> {
...
export async function callUserLlmJson(
  uid: string,
  prompt: string,
  opts?: { systemInstruction?: string; temperature?: number },
): Promise<string> {
  const { providerId, key, model } = await getUserLlmCredentials(uid);
  if (providerId === 'google_gemini') {
    const gen = new GoogleGenerativeAI(key);
...
function toGeminiDeclarations(tools: LlmToolDeclaration[]): FunctionDeclaration[] {
  return tools.map(
    (t) =>
      ({
        name: t.name,
...
function toOpenAiTools(tools: LlmToolDeclaration[]): OpenAI.Chat.ChatCompletionTool[] {
  return tools.map((t) => ({
...
export async function callUserLlmWithTools(
  uid: string,
  opts: {
    systemInstruction: string;
    contents: LlmContentPart[];
    tools: LlmToolDeclaration[];
    temperature?: number;
    maxTokens?: number;
  },
): Promise<LlmCallWithToolsResult> {
  const { providerId, key, model } = await getUserLlmCredentials(uid);
  ...
  const functionCalls: LlmFunctionCall[] = (msg?.tool_calls ?? []).map((tc) => ({
