...
import {
  ...
  isProspectDismissed,
  listActiveProspectsForMaintenance,
  mergeDismissedMaintenanceMeta,
...
function emptyStore(): CerebroStore {
  return {
    version: 1,
...
describe('prospect-dismiss', () => {
  ...
  it('isProspectDismissed matches dismissed id', () => {
    ...
    assert.equal(
      isProspectDismissed(store, { id: 'ghost-id', displayName: 'Anyone', aliases: [] }),
      true,
    );
  });
