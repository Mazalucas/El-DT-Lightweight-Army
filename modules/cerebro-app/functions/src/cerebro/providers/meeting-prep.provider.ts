...
function matchEventByQuery(
  events: Awaited<ReturnType<typeof getCalendarTodayView>>['events'],
  query?: string,
) {
...
export const meetingPrepProvider: CerebroToolProvider = {
  id: 'meeting-prep',
  ...
  declarations: [
    {
      ...
      description:
        'Preparación para una reunión: pendientes, compromisos abiertos, reuniones previas de la misma serie o con los mismos participantes, y preview de notas. Usá esto cuando el usuario pregunte qué entregar, preparar o recordar antes de una reunión.',
      parameters: {
        type: 'object',
        properties: {
          eventTitle: {
  ...
  async execute(ctx, name, args) {
    if (name !== 'get_meeting_prep') throw new Error(`meeting-prep provider: ${name}`);
    ...
    let event =
      (eventId ? calendar.events.find((e) => e.id === eventId) : undefined) ??
    ...
    const notePreviews: Array<{ meetingId: string; title: string; preview: string }> = [];
    if (includePreview) {
...
export function meetingPrepDeclarations(): LlmToolDeclaration[] {
