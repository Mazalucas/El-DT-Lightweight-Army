import type { CerebroContextSnapshot, MomentCardBlock } from '../shared/cerebro-chat.js';
...
export interface ProactiveEvaluationInput {
  ...
  toolRunning?: boolean;
  planPending?: boolean;
  conversationAgeMinutes?: number;
}

function wasDismissed(keys: string[] | undefined, key: string): boolean {
  return (keys ?? []).includes(key);
}

export function evaluateProactiveMoment(input: ProactiveEvaluationInput): MomentCardBlock | undefined {
  const { snapshot } = input;
  const prefs = snapshot.preferences ?? DEFAULT_CEREBRO_PREFERENCES;
  if (prefs.proactiveLevel === 'off') return undefined;
  if (input.userTyping || input.toolRunning || input.planPending) return undefined;
    ...
    if (next.minutesUntil <= prefs.meetingReminderMinutes) {
      ...
      if (!wasDismissed(dismissed, key)) {
        return {
          ...
          actions: [
            ...(next.meetLink
              ? [{ id: 'meet', label: 'Abrir Meet', kind: 'navigate' as const, payload: next.meetLink }]
              : []),
  ...
  if (
    snapshot.workload.overdueTodos >= 3 &&
