...
import type { TodoSourceSection } from './types.js';
...
const FALLBACK_SECTIONS: { name: string; key: TodoSourceSection }[] = [
...
const ACTION_VERB_RE =
  /\b(enviar|mandar|preparar|revisar|agendar|coordinar|confirmar|seguir|contactar|llamar|escribir|completar|terminar|entregar|armar|diseñar|implementar|probar|testear|validar|aprobar|subir|publicar|compartir|sincronizar|actualizar|definir|resolver|investigar|analizar|presentar|cotizar|facturar|pagar|firmar|programar|organizar|planificar|delegar|asignar|priorizar|cerrar|abrir|crear|hacer|verificar|chequear|pedir|solicitar|conseguir|obtener|agendar|reprogramar|follow.?up|follow up)\b/i;

const NON_ACTION_LINE_RE =
  /^(se (discuti[oó]|habl[oó]|present[oó]|revis[oó]|acord[oó]|mencion[oó])|la reuni[oó]n|particip(aron|antes)|asist(ieron|encia)|duraci[oó]n|fecha:|equipo:|proyecto:|resumen|overview|temas tratados|asistentes|invitados|notas generales|contexto:|objetivo de la reuni)/i;
...
function isLikelyActionItem(text: string, strict = false): boolean {
  const t = text.trim();
  if (t.length < 10 || t.length > 400) return false;
  if (NON_ACTION_LINE_RE.test(t)) return false;
  ...
  if (strict) return hasVerb || hasDeadlineHint;
  return hasVerb || hasDeadlineHint || t.includes(':') === false;
}

function parseSectionLines(section: string, strict = false): string[] {
  if (!section) return [];
  const lines: string[] = [];
  for (const raw of section.split(/\n+/)) {
    const text = normalizeLine(raw);
    if (!text || !isLikelyActionItem(text, strict)) continue;
    lines.push(text);
  }
  return lines;
}
...
export function parseDueAtFromText(raw: string, referenceDate = new Date()): string | undefined {
  ...
  if (/\b(hoy|para hoy)\b/.test(lower)) return ref.toISOString();
  ...
  for (const [name, dow] of Object.entries(WEEKDAY_ES)) {
    if (lower.includes(name)) return nextWeekday(ref, dow).toISOString();
  }
