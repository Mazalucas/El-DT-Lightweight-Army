import type { CerebroStore, Meeting, MeetingTodo, Person } from './types.js';
import { coerceString } from '../../lib/text-coerce.js';
import {
  extractActionItemsFromBody,
  normalizeTextForId,
  parseActionItemLine,
  parseDueAtFromText,
  todoStableId,
...
function isManualTodo(todo: MeetingTodo): boolean {
  return todo.source === 'manual' || todo.id.startsWith('manual-');
}

export function upsertExtractedTodoInStore(
  store: CerebroStore,
  item: ExtractedTodoDto,
...
export function syncExtractedTodosInStore(
  store: CerebroStore,
  items: ExtractedTodoDto[],
): number {
  let upserted = 0;
...
export function collectExtractedTodosFromMirrors(
  mirrors: { id: string; content: string }[],
  meetings: Meeting[],
): ExtractedTodoDto[] {
  const meetingsById = new Map(meetings.map((m) => [m.id, m]));
  const items: ExtractedTodoDto[] = [];
  ...
  for (const file of mirrors) {
    ...
    for (const { text, sourceSection } of extracted) {
      ...
      if (seen.has(`${meetingId}:${norm}`)) continue;
...
export function upsertAiActionItemsAsTodos(
  store: CerebroStore,
  meetingId: string,
  actionItems: string[],
): number {
...
export interface SmartTodoCandidate {
  text: string;
  meetingId?: string;
  meetingTitle?: string;
...
export function materializeSmartTodoCandidatesInStore(
  store: CerebroStore,
  candidates: SmartTodoCandidate[],
): number {
  let n = 0;
