...
export interface UiTargetDef {
  ...
  description: string;
  routes: string[] | '*';
  navigate?: { hash: string; settingsSection?: string };
  keywords: string[];
}

export const CEREBRO_UI_REGISTRY: UiTargetDef[] = [
  {
    id: 'sync.run_button',
...
/** Plantillas de entidades de dominio — DOM: data-cerebro-entity="{kind}:{id}" */
export interface EntityTemplateDef {
  id: string;
  kind: import('./cerebro-elements.js').EntityKind;
  label: string;
  routes: string[] | '*';
  domAttribute: 'data-cerebro-entity';
}

export const CEREBRO_ENTITY_REGISTRY: EntityTemplateDef[] = [
...
export function getEntityTemplate(id: string): EntityTemplateDef | undefined {
  return CEREBRO_ENTITY_REGISTRY.find((t) => t.id === id);
}

export function getUiTarget(id: string): UiTargetDef | undefined {
  return CEREBRO_UI_REGISTRY.find((t) => t.id === id);
}

export function listUiTargetsForRoute(route: string): UiTargetDef[] {
  return CEREBRO_UI_REGISTRY.filter((t) => t.routes === '*' || t.routes.includes(route));
}

export function searchUiTargets(query: string, route?: string): UiTargetDef[] {
