...
import { acceptProjectSuggestionForOrg, acceptTeamSuggestionForOrg, dismissSuggestionForOrg, } from '../services/org-catalog.js';
...
orgRouter.patch('/:orgId', async (req, res, next) => {
    try {
        const uid = getUid(req);
        const { name, domains, joinPolicy, branding } = req.body;
...
orgRouter.post('/:orgId/catalog/todos', async (req, res, next) => {
    try {
        const uid = getUid(req);
        const result = await createTodoForOrg(req.params.orgId, uid, req.body);
        res.status(201).json(result);
    }
    catch (e) {
        next(e);
...
orgRouter.get('/:orgId/views/people', async (req, res, next) => {
    try {
        ...
        await requireOrgMember(req.params.orgId, uid);
        const store = await loadOrgStore(req.params.orgId);
        res.json(buildPeopleViewFromStore(store, {
            q: typeof req.query.q === 'string' ? req.query.q : undefined,
        }));
    }
    catch (e) {
