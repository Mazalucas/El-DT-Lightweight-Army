...
function pickValidTimezone(...candidates) {
...
function localeFromSettings(settings) {
    return settings.locale;
}
/** Resuelve la zona horaria efectiva según preferencias del usuario. */
export function resolveUserTimezone(settings, clientTz) {
    const locale = localeFromSettings(settings);
    const source = locale?.timezoneSource ?? 'device';
...
export function defaultLocaleSettings(syncScheduleTz) {
