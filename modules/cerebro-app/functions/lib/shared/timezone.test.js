...
import { DEFAULT_APP_TIMEZONE, defaultLocaleSettings, isValidIanaTimezone, resolveUserTimezone, } from './timezone.js';
describe('timezone', () => {
    ...
    it('resolves device source with client TZ', () => {
        const settings = {
            ...DEFAULT_SETTINGS,
            locale: { timezoneSource: 'device', timezone: 'Europe/Madrid' },
        };
        assert.equal(resolveUserTimezone(settings, 'America/New_York'), 'America/New_York');
    });
    ...
    it('resolves manual source ignoring client', () => {
        const settings = {
            ...DEFAULT_SETTINGS,
            locale: { timezoneSource: 'manual', timezone: 'Europe/Paris' },
        };
        assert.equal(resolveUserTimezone(settings, 'America/New_York'), 'Europe/Paris');
    });
