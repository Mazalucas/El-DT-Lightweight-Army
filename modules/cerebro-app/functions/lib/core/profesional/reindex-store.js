...
function indexPeopleByEmail(people) {
    const map = new Map();
    for (const p of people) {
        for (const e of p.emails ?? [])
            map.set(e.toLowerCase(), p);
    }
    return map;
}
function dedupeMirrorFilesByDocId(files) {
    ...
    const docMeetingIds = new Set([...byDocId.values()].map((f) => {
...
export function reindexStoreFromMirrors(store, files) {
    ...
    const prospectResolver = new ProspectResolver(existingProspects.filter((p) => !p.linkedPersonId), store.dismissedProspectKeys ?? [], store.dismissedProspectIds ?? []);
    ...
    let meetingCount = 0;
    const writtenMeetingIds = new Set();
    for (const file of uniqueFiles) {
        ...
        const seenAt = resolveMeetingStartedAt({
        ...
        const personIds = [];
        const prospectIds = [];
        const teamIdsFromEmails = [];
        const tagTeamEmail = (rawEmail) => {
            const teamId = teamEmailIndex.get(rawEmail.toLowerCase().trim());
            if (!teamId)
                return false;
            if (!teamIdsFromEmails.includes(teamId))
                teamIdsFromEmails.push(teamId);
            return true;
        };
        for (const signal of signals) {
            if (signal.email) {
                if (tagTeamEmail(signal.email))
                    continue;
                ...
                const pid = prospectResolver.record(signal.name, meetingId, signal.source, signal.name);
                if (pid && !prospectIds.includes(pid))
                    prospectIds.push(pid);
            }
        }
        for (const email of participantEmails) {
            if (tagTeamEmail(email))
                continue;
        ...
        const meeting = enrichMeetingRecord({
            id: meetingId,
            ...
            lastSyncedAt: typeof fm.syncedAt === 'string'
                ? fm.syncedAt
    ...
    for (const existing of existingAll) {
        if (!isValidContact(existing))
            continue;
        if (nextPeople.has(existing.id))
            continue;
...
function inferProjectNamesFromTitle(title) {
    const found = [];
    const blob = title.toLowerCase();
    for (const hint of PROJECT_HINTS) {
        if (blob.includes(hint.toLowerCase().replace('ø', 'o')))
            found.push(hint);
    }
    return [...new Set(found)];
}
function resolveProjectIdsForMeeting(store, meetingId, title, map) {
    const ids = [];
...
function mergeIds(fromMirror, existing) {
    return [...new Set([...(existing ?? []), ...fromMirror])];
}
function inferTeams(title, teams) {
    const ids = [];
    for (const t of teams) {
        if (title.toLowerCase().includes(t.name.toLowerCase()))
            ids.push(t.id);
    }
    return ids;
}
function buildSearchPreview(body) {
