...
export function dedupeRepeatedNameTokens(name) {
    const words = name.trim().split(/\s+/);
    if (words.length >= 4 && words.length % 2 === 0) {
        const half = words.length / 2;
        const first = words.slice(0, half).join(' ');
        const second = words.slice(half).join(' ');
        if (normalizePersonNameKey(first) === normalizePersonNameKey(second))
            return first;
    }
    return name.trim();
}
/** Quita contexto de chip / artefactos de Docs sobre el nombre de persona. */
export function cleanChipPersonName(raw) {
...
export function isChipLabelVariant(raw, cleaned) {
...
export function personNameCandidates(raw) {
    ...
    const push = (n) => {
        ...
        if (!t)
            return;
        if (!out.some((x) => normalizePersonNameKey(x) === normalizePersonNameKey(t)))
            out.push(t);
    };
    push(trimmed);
    push(cleaned);
...
export function pickPersonDisplayName(rawName, email) {
    if (rawName?.trim()) {
...
const AGENDA_TOPIC_START = /^(problemas|protocolos|presentaci[oó]n|propuesta|situaci[oó]n|rendimiento|simplificaci[oó]n|pr[oó]ximos pasos|discusi[oó]n|descripci[oó]n|planificaci[oó]n|seguimiento|actualizaci[oó]n|contexto|objetivos?|conclusiones?|acuerdos?|decisiones?|riesgos?|bloqueos?|blockers?|demostraci[oó]n|recap|overview|agenda|tema|temas|entregables?|evaluaciones?|contrataciones?|programaci[oó]n|inteligencia artificial|estado del|status del|update del|updates del)\b/i;
/** Vocabulario típico de temas de reunión (no nombres). */
const TOPIC_VOCABULARY = /\b(entregables?|miniaplicaci[oó]n|triggers?|contrataciones?|inteligencia artificial|entregable|turbo|dpx|nexolaps?|vitales|deep links?|miniaplicacion)\b/i;
function isSectionLabelPrefix(key) {
    for (const blocked of SECTION_LABEL_BLOCKLIST) {
        const norm = normalizePersonNameKey(blocked);
        if (key === norm || key.startsWith(`${norm} `) || key.startsWith(`${norm} y `))
            return true;
    }
    return false;
}
...
export function isLikelyPersonName(raw) {
    ...
    if (name.length < 2 || name.length > MAX_PERSON_NAME_CHARS)
        return false;
    if (/^\d/.test(name))
        return false;
    if (/["'«»]/.test(name))
    ...
    if (SECTION_LABEL_BLOCKLIST.has(key))
        return false;
    if (isSectionLabelPrefix(key))
        return false;
    const words = name.split(/\s+/).filter(Boolean);
    if (words.length > MAX_PERSON_NAME_WORDS)
        return false;
    if (words.length === 1 && words[0].length > 20)
        return false;
    if (TOPIC_LINE_PREFIX.test(name))
        return false;
    if (AGENDA_TOPIC_START.test(name))
        return false;
    if (TECH_TITLE_TOKEN.test(name))
        return false;
    if (TOPIC_VOCABULARY.test(name))
        return false;
    return true;
}
