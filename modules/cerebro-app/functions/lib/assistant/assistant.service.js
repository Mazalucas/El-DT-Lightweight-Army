import { callUserLlmText, callUserLlmWithTools, userHasLlmKey, } from '../services/llm-service.js';
...
function buildContextInstruction(pageContext) {
    ...
    if (pageContext.userName)
        lines.push(`- Nombre: ${pageContext.userName}`);
    if (pageContext.userEmail)
        lines.push(`- Email: ${pageContext.userEmail}`);
    if (pageContext.orgName)
        lines.push(`- Organización activa: ${pageContext.orgName}`);
    if (pageContext.orgRole)
        lines.push(`- Rol en la org: ${pageContext.orgRole}`);
    if (pageContext.pageTitle)
        lines.push(`- Página actual: ${pageContext.pageTitle}`);
    if (pageContext.pageDescription)
        lines.push(`- Detalle: ${pageContext.pageDescription}`);
    if (pageContext.meetingId)
        lines.push(`- ID reunión visible: ${pageContext.meetingId}`);
    if (pageContext.profTab)
        lines.push(`- Pestaña profesional: ${pageContext.profTab}`);
    lines.push('Saludá al usuario por su nombre cuando sea natural. Adaptá ejemplos y sugerencias a la página donde está.');
    return lines.join('\n');
}
export async function runAssistantChat(opts) {
    ...
    if (!(await userHasLlmKey(uid))) {
        const err = 'Configurá tu API key de IA en Ajustes → IA antes de usar el asistente.';
    ...
    while (rounds < MAX_TOOL_ROUNDS) {
        ...
        if (result.functionCalls?.length) {
            const modelParts = result.functionCalls.map((fc) => ({
