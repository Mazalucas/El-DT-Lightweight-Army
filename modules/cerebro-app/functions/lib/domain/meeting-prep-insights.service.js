...
function buildPersonEmailIndex(people) {
    const map = new Map();
    for (const p of people) {
        for (const raw of p.emails) {
            const email = raw.toLowerCase().trim();
            if (email && !looksLikeTeamEmail(email))
                map.set(email, p.id);
        }
    }
    return map;
...
function resolvePersonIdsForEvent(event, store, emailIndex) {
    const ids = new Set();
    ...
    if (event.linkedMeetingId) {
        ...
        for (const pid of linked?.personIds ?? [])
            ids.add(pid);
    }
    return [...ids];
}
function openTodosForPerson(store, personId) {
    return store.todos
        .filter((t) => (t.status === 'open' || t.status === 'suggested') &&
        (t.assigneePersonIds?.includes(personId) || t.personIds.includes(personId)))
        .sort((a, b) => (a.dueAt ?? '9999').localeCompare(b.dueAt ?? '9999'));
}
function pastMeetingsWithPersonOverlap(store, personIds, beforeMs, excludeMeetingId) {
    if (!personIds.length)
        return [];
    ...
    return sortMeetingsByRecency(enrichMeetings(store.meetings)).filter((m) => {
        ...
        if (!started || new Date(started).getTime() >= beforeMs)
            return false;
        return m.personIds.some((pid) => set.has(pid));
    });
}
function pastMeetingsByNormalizedTitle(store, normalizedTitle, beforeMs) {
    if (!normalizedTitle)
        return [];
    return sortMeetingsByRecency(enrichMeetings(store.meetings)).filter((m) => {
        const started = resolveMeetingStartedAt(m);
...
function sharedProjectIds(store, personIds) {
...
function prioritizeEvents(events, now) {
...
export function buildMeetingPrepFacts(calendar, store, operatorEmail) {
    if (!calendar.hasCalendarAccess || !calendar.events.length)
        return [];
    ...
    for (const event of prioritized) {
        ...
        if (event.linkedMeetingId) {
            const linked = store.meetings.find((m) => m.id === event.linkedMeetingId);
            for (const pid of linked?.projectIds ?? [])
                projectIds.add(pid);
        }
        for (const pid of sharedProjectIds(store, personIds))
            projectIds.add(pid);
        if (projectIds.size) {
            const projList = [...projectIds];
            const relatedMeetings = sortMeetingsByRecency(enrichMeetings(store.meetings)).filter((m) => m.projectIds.some((p) => projectIds.has(p)) && resolveMeetingStartedAt(m) && new Date(resolveMeetingStartedAt(m)).getTime() < eventMs);
        ...
        if (event.linkedMeetingId) {
            const fromMeeting = store.todos.filter((t) => t.meetingId === event.linkedMeetingId &&
...
function buildEvidenceForFact(store, f) {
    const evidence = [];
    const seen = new Set();
...
function buildEvidence(store, facts) {
    const evidence = [];
    const seen = new Set();
    for (const f of facts) {
...
export function buildFactChip(f, store, event) {
...
export function buildTemplateMeetingPrepInsights(facts, events, store) {
    if (!facts.length)
        return [];
...
export function mergeMeetingPrepInsights(_raw, facts, events, store) {
    return buildTemplateMeetingPrepInsights(facts, events, store);
}
/** Calcula insights en vivo desde calendario + store. */
export function computeLiveMeetingPrepInsights(calendar, store, operatorEmail) {
