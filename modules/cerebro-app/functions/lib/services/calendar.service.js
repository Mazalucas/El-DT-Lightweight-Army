...
import { formatGoogleApiError, getGoogleClient, hasCalendarScope, hasGoogleIntegration, CALENDAR_SCOPE, } from './google.js';
...
function findLinkedMeeting(eventTitle, startAt, meetings) {
    ...
    for (const m of meetings) {
        ...
        if (!mt.includes(title) && !title.includes(mt))
            continue;
        const score = diff;
...
function eventStatus(startAt, endAt, now) {
...
function mapEvent(raw, meetings, now, operatorEmail) {
    ...
    const attendeeEmails = (raw.attendees ?? [])
        ...
        .filter((e) => Boolean(e && e.includes('@') && e !== opEmail));
    const recurrence = raw.recurrence?.filter(Boolean);
...
export async function getCalendarTodayView(uid, opts) {
...
export async function fetchPrimaryCalendarTimezone(uid) {
    const connected = await hasGoogleIntegration(uid);
    if (!connected)
        return null;
    const calendarGranted = await hasCalendarScope(uid);
