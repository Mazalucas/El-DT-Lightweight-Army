...
export function compactStoreForPersist(store) {
    return {
        ...store,
        ...
        meetings: store.meetings.map((m) => ({
            ...
            actionItems: m.actionItems?.length
                ? coerceStringArray(m.actionItems, MAX_ACTION_ITEM).slice(0, MAX_ACTION_ITEMS)
                : undefined,
            participants: coerceStringArray(m.participants, MAX_PARTICIPANT_NAME).slice(0, MAX_PARTICIPANTS),
        })),
...
export function estimateFirestoreJsonBytes(value) {
