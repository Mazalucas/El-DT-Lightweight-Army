...
function coolingRelationships(store) {
    ...
    for (const m of store.meetings) {
        ...
        for (const pid of m.personIds) {
            ...
            if (!prev)
                byPerson.set(pid, { count: 1, last: m.startedAt });
            else
                byPerson.set(pid, { count: prev.count + 1, last: m.startedAt > prev.last ? m.startedAt : prev.last });
        }
    }
    ...
    for (const [pid, info] of byPerson) {
        if (info.count >= 3 && info.last < cutoff) {
            ...
            if (person)
                result.push({ name: person.displayName, personId: pid, lastMeeting: info.last, count: info.count });
        }
    }
    return result.sort((a, b) => b.count - a.count).slice(0, 5);
}
async function buildContext(uid, store) {
    const meetings = recentMeetings(store);
    const todosByMeeting = new Map();
    ...
    for (const m of meetings) {
        ...
        if (m.summary)
            lines.push(`Resumen IA: ${m.summary}`);
        if (m.actionItems?.length)
            lines.push(`Action items del doc: ${m.actionItems.join(' | ')}`);
...
function sanitizeSuggestion(raw, now) {
    const kind = VALID_KINDS.includes(raw.kind) ? raw.kind : null;
    const title = raw.title?.trim();
    const reason = raw.reason?.trim();
    if (!kind || !title || !reason)
        return null;
    const actionKind = raw.action?.kind;
    const action = actionKind === 'create_todo' || actionKind === 'open_meeting' || actionKind === 'open_person'
...
async function listRecentlyHandled(uid) {
...
export async function runSuggestionEngine(uid) {
    if (!(await userHasLlmKey(uid)))
        return { generated: 0, skipped: 'no_llm' };
    const store = await loadStore(uid);
    if (!store.meetings.length)
        return { generated: 0, skipped: 'no_data' };
    ...
    for (const r of parsed.suggestions ?? []) {
        ...
        seen.add(key);
        fresh.push(s);
        if (fresh.length >= MAX_SUGGESTIONS)
            break;
    }
...
export async function generateDailyDigest(uid) {
    if (!(await userHasLlmKey(uid)))
        return null;
    const store = await loadStore(uid);
...
export async function runIntelligence(uid) {
    let suggestions = 0;
    let digest = false;
    try {
