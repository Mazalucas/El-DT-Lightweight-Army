...
export function mergeMemberStoreIntoOrg(orgStore, personal, uid) {
    ensurePendingSuggestions(orgStore);
    ensurePendingSuggestions(personal);
    let merged = 0;
    const meetingByDoc = new Map();
    orgStore.meetings.forEach((m, i) => {
        if (m.docId)
            meetingByDoc.set(m.docId, i);
    });
    for (const m of personal.meetings) {
        ...
        if (m.docId && meetingByDoc.has(m.docId)) {
            ...
            orgStore.meetings[idx] = {
                ...
                prospectIds: [...new Set([...(existing.prospectIds ?? []), ...(m.prospectIds ?? [])])].filter((id) => !isProspectIdDismissed(orgStore, id) && !isProspectIdDismissed(personal, id)),
            ...
            orgStore.meetings.push({
                ...m,
                contributorUids,
                prospectIds: (m.prospectIds ?? []).filter((id) => !isProspectIdDismissed(orgStore, id) && !isProspectIdDismissed(personal, id)),
            });
            if (m.docId)
                meetingByDoc.set(m.docId, orgStore.meetings.length - 1);
        }
    }
    ...
    for (const p of orgStore.people) {
        for (const e of p.emails ?? [])
            peopleByEmail.set(e.toLowerCase(), p.id);
    ...
    for (const pr of personal.prospects) {
        if (isProspectDismissed(personal, pr) || isProspectDismissed(orgStore, pr))
            continue;
    ...
    for (const p of orgStore.projects)
        projectsBySlug.set(slugId(p.name), p.id);
    ...
    for (const t of personal.teams) {
        if (!orgStore.teams.some((x) => x.id === t.id))
            orgStore.teams.push(t);
    }
