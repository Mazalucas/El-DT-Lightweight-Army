...
export function rankProspectLinkCandidates(store, prospectId, limit = 8) {
    ...
    for (const person of store.people.filter(isValidContact)) {
        ...
        for (const n of names) {
            ...
            if (key === prospectKey)
                score += 50;
            else if (key.includes(prospectKey) || prospectKey.includes(key))
                score += 25;
        }
        let sharedMeetings = 0;
        for (const m of store.meetings) {
            if (!m.personIds.includes(person.id))
                continue;
            if (prospectMeetings.has(m.id) || m.prospectIds?.includes(prospectId))
                sharedMeetings++;
        }
...
export function findPersonByNormalizedName(store, name) {
