...
function remapProjectId(store, fromId, toId) {
    let touches = 0;
    for (const m of store.meetings) {
        if (!m.projectIds.includes(fromId))
            continue;
        m.projectIds = [...new Set(m.projectIds.map((id) => (id === fromId ? toId : id)))];
        touches++;
    }
    for (const t of store.todos) {
        if (!t.projectIds.includes(fromId))
            continue;
        t.projectIds = [...new Set(t.projectIds.map((id) => (id === fromId ? toId : id)))];
    }
    for (const p of store.people) {
        if (!p.projectIds?.includes(fromId))
            continue;
        p.projectIds = [...new Set(p.projectIds.map((id) => (id === fromId ? toId : id)))];
    }
    return touches;
}
export function dedupeProjectsInStore(store) {
    ensurePendingSuggestions(store);
    let merged = 0;
    let suggestions = 0;
    const slugForName = (name) => slugId(name.trim());
    const ensureCanonical = (name) => {
        const id = slugForName(name);
        let existing = store.projects.find((p) => p.id === id);
        if (!existing) {
            existing = store.projects.find((p) => !isUuidProjectId(p.id) && p.name.toLowerCase().trim() === name.toLowerCase().trim());
        }
        if (!existing) {
...
export async function repairUserStore(uid) {
...
export async function runRepairUserStore(uid, options) {
...
export async function repairOrgStore(orgId) {
...
export async function runRepairOrgStore(orgId, requesterUid) {
    const members = await listOrgMembers(orgId);
