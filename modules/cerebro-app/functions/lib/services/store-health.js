...
export function computeStoreHealth(store) {
    ...
    for (const m of store.meetings) {
        ...
        if (rawLinks > validPeople + validProspects)
            staleParticipantLinks++;
        if (validPeople + validProspects === 0 && hasNames)
    ...
    const projectIdsUsed = new Set();
    for (const m of store.meetings) {
        for (const pid of m.projectIds)
            projectIdsUsed.add(pid);
    }
    ...
    const orphanProjects = store.projects.filter((p) => !projectIdsUsed.has(p.id) || isUuidProjectId(p.id)).length;
    const needsRepair = (meetingsSynced > 0 && store.people.filter(isValidContact).length === 0) ||
        uuidProjects > 20 ||
