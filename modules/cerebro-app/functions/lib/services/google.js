...
export function getStoredScopes(data) {
    return data?.scopes?.length ? data.scopes : DRIVE_SCOPES;
}
export function hasCalendarScopeFromData(data) {
    return getStoredScopes(data).includes(CALENDAR_SCOPE);
}
export async function hasCalendarScope(uid) {
    const snap = await googleIntegrationRef(uid).get();
    if (!snap.exists)
        return false;
    return hasCalendarScopeFromData(snap.data());
}
export function getAuthUrl(state, opts) {
...
export async function saveGoogleTokens(uid, tokens, opts) {
...
export async function revokeGoogle(uid) {
    await googleIntegrationRef(uid).delete();
}
export async function hasGoogleIntegration(uid) {
    const snap = await googleIntegrationRef(uid).get();
    if (!snap.exists)
        return false;
...
function decryptGoogleTokens(data) {
    try {
        ...
        throw new Error('No se pudieron leer los tokens de Google. Desconectá y volvé a conectar en Ajustes (p. ej. si cambió ENCRYPTION_KEY).');
    }
}
export function formatGoogleApiError(e) {
...
export async function getGoogleClient(uid) {
    const snap = await googleIntegrationRef(uid).get();
    if (!snap.exists)
...
export async function getGoogleAccessToken(uid) {
...
export function getGooglePickerConfig() {
    const clientId = process.env.GOOGLE_OAUTH_CLIENT_ID;
    if (!clientId)
        throw new Error('GOOGLE_OAUTH_CLIENT_ID must be set');
    ...
    if (!appId)
        throw new Error('Could not derive Google Cloud project number for Picker appId');
    return { apiKey, appId, clientId };
}
export async function listDriveFolders(uid, parentId = 'root', query, options) {
    ...
    const listParams = options?.sharedWithMe || parentId !== 'root'
        ? {
            q,
            ...
            ...(options?.sharedWithMe ? { corpora: 'user' } : {}),
        }
        : {
            q,
            fields: 'files(id,name,mimeType,modifiedTime,parents)',
...
export async function suggestMeetFolders(uid) {
    const seen = new Set();
    const out = [];
...
export async function fetchDrivePermissions(uid, fileId) {
    try {
        const auth = await getGoogleClient(uid);
        const drive = google.drive({ version: 'v3', auth: auth });
...
export async function testDriveFolder(uid, folderId) {
    const auth = await getGoogleClient(uid);
    const drive = google.drive({ version: 'v3', auth: auth });
...
export async function fetchDocParsed(uid, docId) {
    const auth = await getGoogleClient(uid);
    const docs = google.docs({ version: 'v1', auth: auth });
    const doc = await docs.documents.get({ documentId: docId });
    return googleDocToParsed(doc.data);
}
export async function fetchDocText(uid, docId) {
    const parsed = await fetchDocParsed(uid, docId);
    return buildMarkdownBody(parsed);
}
export async function uploadToDriveFolder(uid, folderId, fileName, mimeType, buffer) {
    const auth = await getGoogleClient(uid);
    const drive = google.drive({ version: 'v3', auth: auth });
