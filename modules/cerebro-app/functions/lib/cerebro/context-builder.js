...
import { isChipEligible, minutesRemaining, minutesUntil, shouldIncludeCalendarInPrompt, } from './calendar-rules.js';
function buildCalendarContext(calendarView, now) {
...
async function buildWorkload(uid) {
...
export function buildSituationalPromptLayer(snapshot) {
    ...
    if (calendar.ongoingEvent) {
        lines.push(`- Calendario: en curso «${calendar.ongoingEvent.title}» (${calendar.ongoingEvent.minutesRemaining} min restantes)`);
    }
        ...
        const linked = calendar.nextEvent.linkedMeetingId
            ...
            : '';
        lines.push(`- Calendario (solo contexto automático de hoy): próxima «${calendar.nextEvent.title}» en ${calendar.nextEvent.minutesUntil} min${linked}`);
        lines.push(`- Si pregunta qué preparar/entregar para esa reunión: llamá get_meeting_prep (eventTitle="${calendar.nextEvent.title}") antes de decir que no podés.`);
    }
    else if (calendar.hasAccess) {
        lines.push('- Calendario (solo hoy en este snapshot): sin eventos próximos en ventana activa — otras fechas vía get_calendar_today(date=...)');
    }
    else {
        lines.push('- Calendario en snapshot: sin acceso aparente — igual llamá get_calendar_today si preguntan por agenda antes de negar');
    }
    lines.push(`- Carga: ${workload.overdueTodos} tareas vencidas · ${workload.pendingSuggestions} sugerencias · sync ${workload.syncStale ? 'atrasado' : 'ok'}`, '', 'Este bloque es un snapshot parcial. Usá lo que ya figure acá sin repreguntar.', 'Si la pregunta pide más (otra fecha, notas, contactos, búsqueda): llamá herramientas — no digas que no podés sin intentar.', 'Si habla de otra cosa y la reunión es en ≤15 min, podés mencionarlo con tacto.');
    ...
    if (navigation.focusedEntity) {
        ...
        lines.push(`- Entidad en foco: ${fe.kind} id=${fe.id}${fe.orgId ? ` (org ${fe.orgId})` : ''}${detail} — priorizá tools sobre esta entidad si la pregunta es ambigua.`);
    }
    if (navigation.meetingPrepFocus) {
        const mp = navigation.meetingPrepFocus;
        lines.push(`- Preparación de reunión (desde chip UI): «${mp.eventTitle}» · chip: ${mp.chipLabel} · kind=${mp.factKind} · calendarEventId=${mp.calendarEventId}`);
        lines.push(`- OBLIGATORIO: llamá get_meeting_prep(calendarEventId="${mp.calendarEventId}") antes de responder sobre este contexto.`);
    }
...
export async function buildCerebroContextSnapshot(uid, client) {
    const now = new Date();
...
export function buildContextChip(snapshot) {
    const prefs = snapshot.preferences ?? DEFAULT_PREFS;
    const next = snapshot.calendar.nextEvent;
    if (next &&
        isChipEligible({ startAt: next.startAt, minutesUntil: next.minutesUntil, status: next.status }, snapshot.user.timezone, prefs.chipMeetingMinutesMax)) {
        return {
