...
function inferEntityRefFromStep(tool, args) {
    if (!args)
        return undefined;
    if (tool.includes('todo') && args.todoId)
        return { kind: 'todo', id: String(args.todoId) };
    if (tool === 'highlight_entity' && args.kind && args.id) {
        return { kind: String(args.kind), id: String(args.id) };
    }
    return undefined;
}
function toPlanCard(plan) {
    return {
        type: 'plan_card',
        planId: plan.id,
        title: plan.title,
...
export const plannerProvider = {
    ...
    async execute(ctx, name, args) {
        ...
        if (name === 'propose_action_plan') {
            const plan = await proposePlan(uid, {
                ...
                steps: args.steps?.map((s) => ({
                    ...
                    tool: s.tool,
                    args: s.args ?? {},
                    entityRef: parseEntityRef(s.entityRef) ?? inferEntityRefFromStep(s.tool, s.args ?? {}),
                })) ?? [],
            });
        ...
        if (name === 'confirm_plan') {
            ...
            if (!plan)
                return { error: 'Plan no encontrado o ya ejecutado' };
