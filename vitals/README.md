...
El cuerpo de **todas** las rules se genera desde `vitals/specs/rule-bodies/` con **[scripts/sync-ide.sh](../scripts/sync-ide.sh)** (metadata en `vitals/config/rules-manifest.yaml`). Tras editar un `rule-body`, ejecutá el script y commiteá **tanto** `vitals/` como `.cursor/` / `.agents/` / `.claude/` en el mismo commit.
